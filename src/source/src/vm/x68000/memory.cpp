...
void MEMORY::load_rom_files()
{
	...
	for(int i=0; i<2; i++) {
		...
		if (!rom_loaded[0]) {
			rom_loaded[0] = vm->load_data_from_file(app_path, _T("IPLROM.DAT"), (uint8_t *)&m_rom[0x70000], 0x20000);
			rom_loaded[0] |= vm->load_data_from_file(app_path, _T("iplrom.dat"), (uint8_t *)&m_rom[0x70000], 0x20000);
			if (rom_loaded[0]) {
				swap16_data(&m_rom[0x70000], 0x10000);
			} else {
				rom_loaded[0] = vm->load_data_from_file(app_path, _T("IPLROMXVI.DAT"), (uint8_t *)&m_rom[0x70000], 0x20000);
				rom_loaded[0] |= vm->load_data_from_file(app_path, _T("iplromxvi.dat"), (uint8_t *)&m_rom[0x70000], 0x20000);
				if (rom_loaded[0]) {
					swap16_data(&m_rom[0x70000], 0x10000);
					m_is_xvi = true;
		...
		if (!rom_loaded[1]) {
			rom_loaded[1] = vm->load_data_from_file(app_path, _T("CGROM.DAT"), (uint8_t *)m_rom, 0xc0000);
			rom_loaded[1] |= vm->load_data_from_file(app_path, _T("cgrom.dat"), (uint8_t *)m_rom, 0xc0000);
...
{
	int wait = 0;
	write_data_nw(addr, data, &wait, width);
}

uint32_t MEMORY::read_dma_data_n(uint32_t addr, int width)
{
	int wait = 0;
	return read_data_nw(addr, &wait, width);
}
...
//			u_bg[1][ADDRH_MASK(addrh, 0x4000)] = 0xffff;
	...
	d_cpu->write_signal(SIG_M68K_BUSERR, buserr, 1);
...
//		}
		if ((addr & 0xfffff0) == 0xfffff0) {
			switch(addrh & 7) {
			case 1:
				// INT1 read vector number
				is_8bits = true;
				data = d_board->read_io8(addrh);
				break;
			case 3:
				// INT3 read vector number
				is_8bits = true;
				data = d_dmac->read_io8(addrh);
				break;
			case 5:
				// INT5 read vector number
				is_8bits = true;
				data = d_scc->read_io8(addrh);
				break;
			case 6:
				// INT6 read vector number
				is_8bits = true;
				data = d_mfp->read_io8(addrh);
				break;
			}
...
uint32_t MEMORY::read_gvram(uint32_t addrh)
{
	uint32_t addrg;
	uint32_t data;
...
void MEMORY::save_state(FILEIO *fio)
{
	...
	fio->FputUint8(m_ipl_mapping);
	fio->FputUint8(m_sram_writable);

	fio->FputUint8(m_is_xvi);
