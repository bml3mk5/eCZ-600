...
------------------------------------------------------------------------------
  ...
  【注意】システムフォルダやProgram Filesフォルダ、アプリケーションフォルダ
          にはコピーしないでください。
  ...
  起動しない場合：
    ...
    x68000.logを開いてエラーメッセージを確認してください。
...
「画面(Screen)」メニュー
  ...
  【注意】OpenGLはお使いのマシンの環境によっては起動しない、表示されない、
          動作が遅くなるといった弊害が発生します。
...
○ キー操作説明
  ...
  いくつかのメニューはAltキー(Macではoptionキー)と併用して呼び出せます。
  ※Altキーはキー割り当てダイアログにて変更できます。
  【注意】UbuntuではAltキーがシステム側で使用されるため反応しません。
          キー割り当てダイアログにて別のキーを割り当ててください。

  メニュー名                               ホスト側のキー
  ---------------------------------------- ----------------------------------
  操作(Control)
    電源スイッチ(Power Switch)             Alt + F3
  ...
  画面(Screen)
    ...
    OpenGLを使用(同期)(Use OpenGL(Sync))
                                           Alt + Y (OpenGL Asyncと切替)
    OpenGLを使用(非同期)(Use OpenGL(ASync))
                                           Alt + Y (OpenGL Syncと切替)
    OpenGLフィルタ(OpenGL Filter)          Alt + U
  サウンド(Sound)
    ボリューム...(Volume...)               Alt + V
  オプション(Options)
...
------------------------------------------------------------------------------
 ...
 ○ マウスについて
    ...
    本ソフト以外のアプリケーションを使用する場合は、Alt + Ctrl でマウス機能を
